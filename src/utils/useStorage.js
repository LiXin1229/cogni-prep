...
const isStorageAvailable = (type) => {
  try {
    ...
    const key = `__storage_test__${Math.random().toString(36).substr(2)}`
 ...
 */
export const useStorage = (key, defaultValue, storageType = 'localStorage') => {
  // 检查存储类型是否可用
  const storageAvailable = isStorageAvailable(storageType)
  const storage = storageAvailable ? window[storageType] : null
