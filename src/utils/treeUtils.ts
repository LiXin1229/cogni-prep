...
// 展开或折叠节点
export const toggleFoldedNodes = (treeData: NodeType, node: NodeType) => {
  const newTreeData = clone(treeData)

  const targetId = node.id
  
  // 递归查找并更新目标节点
  const findAndUpdate = (node: NodeType) => {
    if (node.id === targetId) {
      // 根据当前isFolded状态切换
      if (node.isFolded === 0) {
        // 设为子节点数量
...
// 折叠节点
export const removeFoldedNodes = (node: NodeType) => {
...
// 添加节点
export const addChildrenById = (treeData: NodeType, parentId: string, newNodes: NodeType | NodeType[]) => {
  const newTree = clone(treeData)
  // 统一处理为数组，方便后续操作
  const nodesToAdd = Array.isArray(newNodes) ? newNodes : [newNodes]

  // 定义递归函数（处理单个节点及其子节点）
  const findAndAdd = (node: NodeType) => {
    // 若当前节点是目标父节点，添加子节点
    if (node.id === parentId) {
...
export const modifyNode = (treeData: NodeType, targetId: string, newName: string, newFrequency: number) => {
  const newTree = clone(treeData)
...
export const deleteNodeById = (treeData: NodeType, targetId: string, mode: 'node' | 'children' = 'node') => {
  const newTree = clone(treeData)

  // 递归查找并删除节点
  const traverse = (parentNode: NodeType) => {
...
// 查找祖先节点
export const findAncestorsById = (treeData: NodeType, targetId: string) => {
  // 存储查找结果
  const result: { parent: NodeType | null, grandparent: NodeType | null } = {
    parent: null,
    grandparent: null
  }

  // 递归遍历树结构
  const traverse = (node: NodeType, parentNode: NodeType | null, grandparentNode: NodeType | null) => {
...
// 修改树结构中指定ID节点的属性
export const modifyTreeNodeProp = (treeData: NodeType, targetId: string, propName: string, propValue: number | string) => {
